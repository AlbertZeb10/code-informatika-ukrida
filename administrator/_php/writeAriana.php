...
if (isset($_POST['req'])) {
    ...
    if ($req == 'delete') {
        $action = 'delete';
        $sql = "DELETE FROM ariana WHERE id= $id ";
    }
} else {
    $id = $_POST['id'];
    ...
    if ($id == "" || $id == null) {
        $action = "insert";
        if($image=='default'){
            $sql =  "INSERT INTO ariana (name_artist,name_music)
        ...
        $action = "edit";
        if($image=='default'){
        $sql = "UPDATE ariana SET name_artist = '$name_artist', name_music = '$name_music' WHERE id = $id";
        }else{
            $sql = "UPDATE ariana SET name_artist = '$name_artist', name_music = '$name_music' WHERE id = $id";
        }
    }
