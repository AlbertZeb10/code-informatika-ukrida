...
if (isset($_POST['req'])) {
    ...
    if ($req == 'delete') {
        $action = 'delete';
        $sql = "DELETE FROM fiersa WHERE id= $id ";
    }
} else {
    $id = $_POST['id'];
    ...
    if ($id == "" || $id == null) {
        $action = "insert";
        if($image=='default'){
            $sql =  "INSERT INTO fiersa (name_artist,name_music)
        ...
        $action = "edit";
        if($image=='default'){
        $sql = "UPDATE fiersa SET name_artist = '$name_artist', name_music = '$name_music' WHERE id = $id";
        }else{
            $sql = "UPDATE fiersa SET name_artist = '$name_artist', name_music = '$name_music' WHERE id = $id";
        }
    }
